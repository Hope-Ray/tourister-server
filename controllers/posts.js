...
export const updatePost = async (req, res) => {
    const { id } = req.params;   //ID coming from URL eg: /posts/123  mentioned in routes
    const {title, message, creator, selectedFile, tags, locations } = req.body;

    if(!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send('NO post available with that ID');
    
    const updatedPost = {title, message, creator, selectedFile, tags, locations, _id: id };
